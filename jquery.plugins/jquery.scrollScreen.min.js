...
w=function(a){c.debug&&console.log("setCurrentViewport(scrollTop)",a);a=((a||t())+f)/r;a*=f;B(a)},C=function(a){c.debug&&console.log("viewportClick(event)",a);var b=0,b=10>a.clientY?0:a.clientY>f-10?r:r*((a.clientY+k)/f);u(b);return!1},x=function(){c.debug&&console.log("viewportDbClick()");u(1);return!1};c.debug&&console.log("createViewport()");1>v||(h.on("click touchend",C).on("dblclick",x).on("mousemove",z).on("mouseout",m),d.on("click touchend",C),w());var E=function(){c.debug&&console.log("scrollRefreshEvent()");
g=null;d.removeClass("current")},F=function(a){c.debug&&console.log("resizeRefreshEvent()");f=p.height();r=s.height();v=Math.ceil(s.height()/f);w();c.checkHash&&(g=setTimeout(function(a){g=null;D(a)},1E3,a||j()))};p.on("scroll",function(){g&&clearTimeout(g);g=setTimeout(E,600);w();d.addClass("current")}).on("resize",function(){g&&clearTimeout(g);g=setTimeout(F,360,j())});var D=function(a){c.debug&&console.log("checkHash(hash)",a,"getHash()",j());if(a=a||j())if(a=a.split("screen_"),1<parseInt(a[1],
10)){a=a[1];c.debug&&console.log("moveToViewport(view)",a);var b=0,b=a==v?r:a*f;u(b)}};c.checkHash&&D();return this};b.fn.scrollScreen.defaults={debug:"console"in n&&!1,checkHash:!1,scrollScreenZone:"scrollScreenZone",scrollScreen:"scrollScreen"}})(jQuery,window,document);
